...
print("> Extracting individuals metadata...")
...
ind_list[['IID']].to_csv("data/keep_files/ukbb_qc_individuals_all.keep", header=False, index=False)

for pop in ind_list['pop'].unique():

    ind_list.loc[ind_list.pop == pop, ['IID']].to_csv(
...
variant_df[['SNP']].to_csv("data/keep_files/hq_imputed_variants.txt", index=False)

# Restrict to variants that have global MAF >= 0.001:
variant_df.loc[variant_df.MAF >= 0.001, ['SNP']].to_csv(
    "data/keep_files/hq_imputed_variants_maf001.csv", index=False
)

# Restrict to variants in "expanded HapMap3" set from Prive et al.:
hm3_df = pd.read_csv("metadata/hm3_plus_prive.csv.gz")
variant_df.merge(hm3_df, left_on='SNP', right='SNP')[['SNP']].to_csv(
    "data/keep_files/hq_imputed_variants_hm3.csv", index=False
)
