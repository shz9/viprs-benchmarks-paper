...
do
  ...
  score_file=$(echo "$fit_file" | sed "s|data/model_fit|data/score/cartagene|g" | sed 's/\.fit\.gz//g')
  ...
  sbatch -J "${job_name}" \
         2_panukb_analysis/score/cartagene_score/score_cartagene_job.sh "$fit_file" \
         "$score_file"
