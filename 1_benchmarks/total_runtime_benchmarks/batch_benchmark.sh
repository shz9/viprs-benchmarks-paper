...
do
        ...
        do
          ...
          if [ "$m" = "true" ] && [ "$t" -eq 1 ]; then
            ...
            do
              ...
              sbatch -J "new_viprs_fold_${fold}_${model_id}" 1_benchmarks/total_runtime_benchmarks/benchmark_new_viprs.sh "fold_$fold" "$ld" "$t" "$j" "$m" "$q"
            done
          else
            model_id="l${ld}_m${m}_qfalse_t${t}_j${j}_lmo_set_zero"
            sbatch -J "new_viprs_fold_${fold}_${model_id}" 1_benchmarks/total_runtime_benchmarks/benchmark_new_viprs.sh "fold_$fold" "$ld" "$t" "$j" "$m" "false"
          fi
  ...
  sbatch -J "old_viprs_fold_$fold" 1_benchmarks/total_runtime_benchmarks/benchmark_old_viprs.sh "fold_$fold"
