        ...
        do
          ...
          if [ "$m" = "true" ] && [ "$t" -eq 1 ]; then
            ...
            do
              ...
              sbatch -J "new_viprs_bma_fold_${fold}_${model_id}" 1_analysis/total_runtime_benchmarks/test_viprs_bma/benchmark_new_viprs_bma.sh "fold_$fold" "$ld" "$t" "$j" "$m" "$q"
            done
          else
            model_id="l${ld}_m${m}_qfalse_t${t}_j${j}"
            sbatch -J "new_viprs_bma_fold_${fold}_${model_id}" 1_analysis/total_runtime_benchmarks/test_viprs_bma/benchmark_new_viprs_bma.sh "fold_$fold" "$ld" "$t" "$j" "$m" "false"
          fi
